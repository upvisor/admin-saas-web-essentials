...
export const Bloque3: React.FC<Props> = ({ edit, design, index, pages, setPages, i, categories, productsOrder, pageNeed }) => {
  return (
                                                    ...
                                                    <>
                                                      ...
                                                      <button className='bg-[#f6531a] border border-[#f6531a] w-fit m-auto transition-colors duration-200 text-white py-1.5 px-6 hover:bg-transparent rounded-md hover:text-[#f6531a]'>{design.info.button}</button>
                                                      {
                                                        design.info?.image?.url && design.info.image.url !== ''
                                                          ? <Image className='h-fit mx-auto' width={480} height={300} alt='Imagen slider prueba' src={design.info.image.url} />
                                                          : ''
                                                      }
                                                    </>
                                                  )
                                                  : (
                                                    <>
                                                      <input type='text' placeholder='Titulo' value={design.info.title} onChange={(e: any) => {
                                                        ...
                                                        setPages(oldPages)
                                                      }} className='text-[25px] font-medium md:text-[32px] w-[800px] m-auto text-center p-1.5 rounded border' />
                                                      <input type='text' placeholder='Descripción' value={design.info.description} onChange={(e: any) => {
                                                        ...
                                                        setPages(oldPages)
                                                      }} className='font-medium text-sm text-center lg:text-[16px] p-1.5 rounded border' />
