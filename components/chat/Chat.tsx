...
const socket = io(`${process.env.NEXT_PUBLIC_API_ADMIN_URL}/`)

export const Chat = () => {

  const [chatOpacity, setChatOpacity] = useState('-mb-[700px]')
  const [chat, setChat] = useState<IMessage[]>([{
    response: '¡Hola! ¿En que te podemos ayudar?',
    adminView: false,
    userView: false
  }])
  ...
  const getMessages = async () => {
    if (localStorage.getItem('chatId')) {
      ...
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_ADMIN_URL}/chat/${senderId}`)
  ...
  const submitMessage = async (e: any) => {
    ...
    if (chat.length === 1) {
      await axios.post(`${process.env.NEXT_PUBLIC_API_ADMIN_URL}/chat/create`, { senderId: senderId, response: chat[0].response, adminView: false, userView: true })
    }
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_ADMIN_URL}/chat`, { senderId: senderId, message: message, adminView: false, userView: true })
  ...
  return (
    <>
        <div className={`${chatOpacity} fixed bottom-24 right-4 flex z-40 h-[480px] ml-3 justify-between flex-col gap-3 transition-all duration-500 bg-white shadow-md rounded-xl dark:bg-neutral-800 w-96 sm:h-[600px] sm:gap-4`}>
                      ...
                      info.message
                        ? (
                          <div className='flex flex-col gap-2 ml-6'>
                            <div className='bg-gray-200 p-1.5 rounded-md w-fit ml-auto dark:bg-neutral-700'><p>{info.message}</p></div>
                          </div>
                        )
                        : ''
                    }
          ...
          <form className='flex gap-2 pr-3 pl-3 pb-3 sm:pr-4 sm:pl-4 sm:pb-4'>
            <input onChange={inputChange} value={newMessage} type={'text'} placeholder={'Mensaje'} className='p-1.5 w-full rounded-md border transition-colors duration-100 focus:outline-none focus:border-main focus:ring-1 focus:ring-main dark:bg-neutral-800 dark:border-neutral-700' />
            <button type='submit' onClick={submitMessage} className='bg-main text-white w-28 rounded-md border border-main transition-colors duration-200 hover:bg-transparent hover:text-main dark:bg-main'>Enviar</button>
          </form>
        </div>
