...
export const PopupDeleteFunnel: React.FC<Props> = ({ popupDelete, setPopupDelete, setSelectFunnel, selectFunnel, getFunnels }) => {
  ...
  return (
    <div onClick={() => {
        ...
        <div onMouseEnter={() => setPopupDelete({ ...popupDelete, mouse: true })} onMouseLeave={() => setPopupDelete({ ...popupDelete, mouse: false })} className={`${popupDelete.opacity === 'opacity-0' ? 'scale-90' : 'scale-100'} transition-transform duration-200 w-full max-w-[500px] overflow-y-auto p-6 rounded-xl flex flex-col gap-4 m-auto border border-border bg-white dark:bg-neutral-800 dark:border-neutral-700`} style={{ boxShadow: '0px 3px 10px 3px #c1c1c1' }}>
